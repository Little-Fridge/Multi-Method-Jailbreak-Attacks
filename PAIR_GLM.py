...
def main():
    p = argparse.ArgumentParser(description="PAIR攻击：LLaVA-7B(本地分支) -> ChatGLM3-6B")
    p.add_argument("-i","--input",required=True)
    p.add_argument("-o","--output",required=True)
    p.add_argument("--atk_model",default="/share/project/test/backup/arc/ppx/llava-1.5-7b-hf")
    p.add_argument("--tgt_model",default="/share/project/test/backup/arc/ppx/chatglm3-6b")
    ...
    tgt = AutoModel.from_pretrained(
        args.tgt_model, trust_remote_code=True
    ).half().cuda().eval()
    ...
    for uid, cat, base_prompt in load_prompts(args.input):
        ...
        for i in range(1, args.max_iter+1):
            ...
            t0 = time.time()
            try:
                response, _ = tgt.chat(tokT, new_prompt, history=[], max_new_tokens=args.max_new_tokens)
                ok = judge(response)
            except Exception as e:
