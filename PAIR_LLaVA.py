...
def main():
    ...
    atk = LlavaForConditionalGeneration.from_pretrained(
        args.atk_model, quantization_config=bnb,
        trust_remote_code=True
    ).eval()

    print("加载目标模型:", args.tgt_model)
    tokT = AutoTokenizer.from_pretrained(args.tgt_model, use_fast=False, trust_remote_code=True)
    tgt = LlavaForConditionalGeneration.from_pretrained(
        args.tgt_model, quantization_config=bnb,
        trust_remote_code=True
    ).eval()
